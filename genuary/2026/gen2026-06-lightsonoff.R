...
solar_grid_dark <- grid_base |> 
  left_join(shadows, by = c("x", "y")) |> 
  mutate(
    shadow = replace_na(shadow, 0),
    light = 0.15 / (1 + dist_to_light / 5),
    value = floor(pmax(0, light - shadow * 0.5) * n_steps) / n_steps
  )

# Blend Johnson palette colors with dark blue
johnson_colors <- MetBrewer::met.brewer("Johnson")
dark_blue_srgb <- colorspace::hex2RGB(dark_blue)

dark_johnson <- map_chr(johnson_colors, function(color) {
  color_srgb <- colorspace::hex2RGB(color)
  mixed <- colorspace::mixcolor(0.7, color_srgb, dark_blue_srgb)
  colorspace::hex(mixed)
})

ggplot(solar_grid_dark, aes(x, y, fill = value)) +
  geom_tile() +
  geom_tile(data = genuary, fill = light_blue) +
  scale_fill_gradientn(colors = dark_johnson) +
  coord_fixed(expand = FALSE) +
  theme_void() +
  theme(legend.position = "none")
