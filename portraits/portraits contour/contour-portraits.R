...
img <- image_read("portraits/images/audrey.jpg")
...
img_df <- rbind(img_df_r, img_df_g, img_df_b) %>% 
  ...
  mutate(
    ...
    # map c (0-255) to cf (0-1), in order to use for alpha
    cf = c / 255
  )

ggplot(img_df) +
  geom_contour_filled(aes(x = x, y = y, z = cf), color = NA, bins = 4) +
  scale_y_reverse() +
  scale_fill_grey() +
  ...
  ggsave("portraits/portraits contour/plots/audrey.png", dpi = 320, height = 8, width = 6)
